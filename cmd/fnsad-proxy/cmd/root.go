...
import (
	...
	"github.com/Finschia/finschia-proxy/v4/app/params"
...
type TxFilterConfig struct {
	AllowedTargets        []string `mapstructure:"allowed-targets"`
	InitialBlockHeight    int64    `mapstructure:"initial-block-height"`
	AllowedContracts      []string `mapstructure:"allowed-contracts"`
	DisableContractFilter bool     `mapstructure:"disable-contract-filter"`
}
...
type CustomAppConfig struct {
	...
	TxFilter TxFilterConfig `mapstructure:"tx-filter"`
}
...
[tx-filter]
# This is the allow list for TxFilterDecorator.
# The target must be selected using the specified form. (e.g. lbm, lbm.token, lbm.token.v1, lbm.token.v1.MsgSend)
# If you register a allow list at a higher level, the filter at a lower level is ignored.
# Caution: we assume that every module's msgs follow standard cosmos proto path (<namespace>.<module>.<version>.<msg>).
allowed-targets = {{ .TxFilter.AllowedTargets }}

# CosmWasm Contract Address list to allow execution.
# You have to register wasm module on whitelist in advance to use this option.
# If you leave this list blank, the filter will deny all contracts.
allowed-contracts = {{ .TxFilter.AllowedContracts }}

# If it set true, All contracts are allowed regardless of the set value of allowed-contracts.
disable-contract-filter = {{ .TxFilter.DisableContractFilter }}

# This is the initial block height for TxFilterDecorator.
# It is used to apply exceptional rules to various type transactions that run when a Genesis block is created.
initial-block-height = {{ .TxFilter.InitialBlockHeight }}
`

	return customAppTemplate, DefaultCustomAppConfig()
}
