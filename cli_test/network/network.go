...
type (
	...
	// or handler.
	Validator struct {
		AppConfig  *lbmcmd.CustomAppConfig
...
func New(t *testing.T, cfg Config) *Network {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		appCfg := lbmcmd.DefaultCustomAppConfig()
		...
		logger := ostlog.NewNopLogger()
		if cfg.EnableLogging {
			logger = ostlog.NewOCLogger(ostlog.NewSyncWriter(os.Stdout))
			logger, err = ostlog.ParseLogLevel("info", logger, ostcfg.DefaultLogLevel)
			require.NoError(t, err)
		}
...
func (n *Network) Cleanup() {
	...
	for _, v := range n.Validators {
		if v.tmNode != nil && v.tmNode.IsRunning() {
			_ = v.tmNode.Stop() //nolint: errcheck
		}
