...
RUN --mount=type=cache,target=/root/.cache/go-build \
    ...
    GOWORK=off go build \
        ...
        -trimpath \
        -o /finschia-build/finschia/build/fnsad-proxy \
        /finschia-build/finschia/cmd/fnsad-proxy
...
COPY --from=build-env /finschia-build/finschia/build/fnsad-proxy /usr/bin/fnsad-proxy
