...
# store `queue.wasm`
STORE_RES=$(${FNSAD} tx wasm store contracts/queue.wasm --from $FROM_ACCOUNT --home ${CHAIN_DIR} --keyring-backend test --chain-id finschia --gas 1500000 -b block -o json -y)
...
INIT_MSG=$(jq -nc '{}')
INSTANTIATE_RES=$(${FNSAD} tx wasm instantiate "$CODE_ID" "$INIT_MSG" --label $TOKEN_NAME --admin "$(${FNSAD} keys show $FROM_ACCOUNT -a --home ${CHAIN_DIR} --keyring-backend test)" --from $FROM_ACCOUNT --home ${CHAIN_DIR} --keyring-backend test --chain-id finschia -b block -o json -y)
...
for value in 100 200 300; do
	...
	RUN_INFO=$(${FNSAD} tx wasm execute "$CONTRACT_ADDRESS" "$ENQUEUE_MSG" --from $FROM_ACCOUNT --home ${CHAIN_DIR} --keyring-backend test --chain-id finschia -b block -y)
...
RUN_INFO=$(${FNSAD} tx wasm execute "$CONTRACT_ADDRESS" "$DEQUEUE_MSG" --from $FROM_ACCOUNT --home=${CHAIN_DIR} --keyring-backend test --chain-id finschia -b block -y)
check_run_info "$RUN_INFO" "$ENQUEUE_MSG"
