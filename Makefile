...
build-reproducible: go.sum
	mkdir -p $(BUILDDIR)
	$(DOCKER) buildx create --name finschiabuilder-proxy || true
	$(DOCKER) buildx use finschiabuilder-proxy
	$(DOCKER) buildx build \
		...
		-t finschia/finschianode-proxy:local-$(ARCH) \
		--load \
		-f Dockerfile .
	$(DOCKER) rm -f finschiabinary || true
	$(DOCKER) create -ti --name finschiabinary finschia/finschianode-proxy:local-$(ARCH)
	$(DOCKER) cp finschiabinary:/usr/bin/fnsad-proxy $(BUILDDIR)/fnsad-proxy-linux-$(ARCH)
...
docker-build:
	@DOCKER_BUILDKIT=1 docker build \
		-t finschia/finschianode-proxy:local \
		-t finschia/finschianode-proxy:local-distroless \
		--build-arg GO_VERSION=$(GO_VERSION) \
...
localnet-docker-build:
	@DOCKER_BUILDKIT=1 docker build \
    		-t finschia/finschianode-proxy:localnet \
    		--build-arg GO_VERSION=$(GO_VERSION) \
    		--build-arg RUNNER_IMAGE=$(RUNNER_BASE_IMAGE_ALPINE) \
    		--build-arg GIT_VERSION=$(VERSION) \
    		--build-arg GIT_COMMIT=$(COMMIT) \
...
localnet-build-nodes:
	docker run --rm -v $(CURDIR)/mytestnet:/data finschia/finschianode-proxy:localnet \
...
PACKAGE_NAME := github.com/Finschia/finschia-proxy
