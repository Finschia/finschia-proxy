...
func (suite *AnteTestSuite) SetupTest(isCheckTx bool, invCheckPeriod uint) {
	suite.app, suite.ctx = createTestApp(suite.T(), isCheckTx, invCheckPeriod)
	...
	testWasmCfg := wasmtypes.DefaultWasmConfig()
	anteHandler, err := proxyante.NewAnteHandler(
		proxyante.HandlerOptions{
			...
			WasmConfig: &testWasmCfg,
		},
		proxyante.MakeAppOptionsMock(),
	)
...
func (suite *AnteTestSuite) CreateTestAccounts(numAccs int) []TestAccount {
	...
	for i := range accounts {
		...
		someCoins := sdk.Coins{
			sdk.NewInt64Coin(sdk.DefaultBondDenom, 10000000),
		}
		...
		accounts[i] = TestAccount{acc, priv}
	}
